...
const ethAddress = '0x52963EF50e27e06D72D59fcB4F3c2a687BE3cfEf'
const delegatedAddr = newDelegatedEthAddress(
  ethAddress,
  CoinType.TEST
).toString()
...
describe('Send', () => {
  ...
  test.only('it allows a user to send a message to an eth address', async () => {
    const { Tree, walletProvider } = composeMockAppTree('postOnboard')
    let result: RenderResult | null = null
    ...
    await act(async () => {
      ...
      // Enter recipient
      recipient.focus()
      fireEvent.change(recipient, { target: { value: ethAddress } })
      recipient.blur()
      jest.runAllTimers()
    ...
    const message = Message.fromLotusType(lotusMessage)
    expect(message.from).toBe(WALLET_ADDRESS)
    expect(message.to).toBe(delegatedAddr)
    expect(message.nonce).toBeGreaterThanOrEqual(0)
    expect(BigNumber.isBigNumber(message.value)).toBe(true)
    expect(message.value.isEqualTo(validAmount.toAttoFil())).toBe(true)
    expect(message.method).toBe(0)
    ...
    expect(pendingMsg.cid).toBeTruthy()
    expect(pendingMsg.from.robust).toBe(WALLET_ADDRESS)
    expect(pendingMsg.to.robust).toBe(delegatedAddr)
    expect(pendingMsg.height).toBe(0)
    expect(pendingMsg.params).toBe('')
    expect(Number(pendingMsg.nonce)).toBeGreaterThanOrEqual(0)
    expect(Number(pendingMsg.method)).toBe(0)

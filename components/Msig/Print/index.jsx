...
const Print = () => {
  ...
  const onSubmit = async e => {
    ...
    try {
      ...
      const relevantMessages = await getMsgsUntilCustodyTaken(
        messagesWithParams
      )
          ...
          switch (msg.params?.method) {
            ...
            case 6: {
              text = `Removed signer: ${msg.params.params.signer} ${
                PL_SIGNERS.has(msg.params.params.signer)
                  ? '(Protocol Labs)'
                  : ''
              } on ${dayjs
                .unix(msg.timestamp)
  ...
  return (
    ...
    >
      <OnboardCard
        ...
        bg={err && 'card.error.background'}
      >
        {err ? (
          <>
            <Title textAlign='center'>{err}</Title>
            <Button
              variant='secondary'
              title='Retry'
              onClick={() => setErr('')}
          ...
          <>
            <Box>
              ...
              <Text>
                Please input a multisig address below to print its details as a
                PDF.
              </Text>
            </Box>
            <form

...
describe('message confirmer', () => {
  ...
  test('it confirms despite the server returning null', async () => {
    // https://github.com/filecoin-project/lotus/issues/1907
    const requestMock = jest
      .fn()
      .mockImplementationOnce(() => {
        throw new Error(
          'resolution lookup failed (t1ivf2iq3kgzgwgcza6tlxkep2kc3jozlfqwi2gfy): resolve address t1ivf2iq3kgzgwgcza6tlxkep2kc3jozlfqwi2gfy: address not found'
        )
      })
      .mockImplementationOnce(() =>
        Promise.resolve({

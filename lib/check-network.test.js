...
describe('NetworkChecker', () => {
  ...
  test('it resets the wallets in redux after network switch', done => {
    const mockDispatch = jest.fn((...args) => args)
    const query = { network: TESTNET }
    const pathname = '/'
    const networkInRedux = TESTNET // network in redux is testnet
    ...
    setTimeout(() => {
      // this basically says we arent passing any new wallets to redux, since the switchNetwork call does take wallets
      expect(mockDispatch.mock.results[0].value[1]).toBeUndefined()
      expect(mockDispatch.mock.results[0].value[0]).toBe(MAINNET)
      done()
    })
  })
})
